...
* PURPOSE: Data preparation and variable construction for provider choice analysis
* ============================================================================== 
...
* ==============================================================================
* 1. IMPORT AND SAVE SUB-MODULES
* ============================================================================== 

* ------------------------------------------------------------------------------
* 1.1 Household module
* ------------------------------------------------------------------------------
...
* ==============================================================================
* 2. MERGE MODULES (STARTING FROM CHILD MODULE)
* ============================================================================== 
...
* Merge women module to child module
...
* Merge household module to child module
...
* ==============================================================================
* 3. DEFINE ANALYTICAL POPULATION: SICK CHILDREN
* ============================================================================== 

* ARI = cough with fast/difficult breathing

gen ari = (ca16 == 1 & (ca17 == 1 | ca18 == 1))
label variable ari "Acute Respiratory Infection"

* Child considered ill if had diarrhea, fever, or ARI in last 2 weeks
...
* ==============================================================================
* 4. RECODE MICS MISSING VALUE FLAGS
* ============================================================================== 

* Recode standard MICS missing-value categories to Stata missing (.)
* Common special codes: 7, 8, 9, 97, 98, 99
* This section now includes all core analysis variables and optional variables
* used later in the script, wherever those variables exist.

local mics_missing_vars ///
    hh1 hh2 uf4 ///
    chweight hh6 hh7 ///
...
foreach var of local mics_missing_vars {
    capture confirm variable `var'
    if !_rc {
        capture confirm numeric variable `var'
        if !_rc {
            replace `var' = . if inlist(`var', 7, 8, 9, 97, 98, 99)
        }
        else {
            replace `var' = "" if inlist(`var', "7", "8", "9", "97", "98", "99")
            replace `var' = "" if inlist(`var', "D", "DK", "NA", "N/A", ".")
        }
    }
}

di "MICS missing flags recoded for all available analysis variables."


* ==============================================================================
* 5. CONVERT PROVIDER VARIABLES TO NUMERIC 0/1
* ============================================================================== 
...
* ==============================================================================
* 6. GENERATE DEPENDENT VARIABLES
* ============================================================================== 

* ------------------------------------------------------------------------------
* 6.1 Multinomial outcome: provider_choice
* 0 = No care sought
* 1 = Public provider
* 2 = Private provider
...
* C = Govt Health Post
* D = Community Health Worker (Govt)
...
* ------------------------------------------------------------------------------
* 6.2 Binary outcome: care_sought
* 0 = No care sought
* 1 = Any care sought
* ------------------------------------------------------------------------------

gen byte care_sought = (provider_choice > 0) if !missing(provider_choice)

capture label drop care_sought_lbl
label define care_sought_lbl 0 "No care sought" 1 "Care sought"
label values care_sought care_sought_lbl
label variable care_sought "Any healthcare sought for sick child (1=Yes, 0=No)"

di "Care sought distribution:"
tab care_sought, missing

di "Cross-tabulation: care_sought vs provider_choice"
tab care_sought provider_choice, missing
...
* Child functional difficulty (ages 24+ months)
...
* Recode ha2-ha6 if they are string
...
* Recode autonomy variables if stored as string
...
* ------------------------------------------------------------------------------
* 7.3 Household and socioeconomic variables
* ------------------------------------------------------------------------------
...
if !_rc {
    ...
    gen crowding_index = hh48 / hh12_num if hh12_num > 0 & !missing(hh48, hh12_num)
    drop hh12_num
}
else {
...
* ------------------------------------------------------------------------------
* 7.8 Composite: care_engagement score (0-5)
* ------------------------------------------------------------------------------
...
* ==============================================================================
* 7B. KEEP ONLY VARIABLES THAT EXIST
* ============================================================================== 
...
foreach var in ///
    ...
    ma11 mt10 mt11 ///
    ws1 ws11 hh48 ///
    provider_choice care_sought ari is_ill ///
...
* PUBLIC MEDICAL SECTOR
capture label drop provA_lbl
label define provA_lbl 0 "Not mentioned" 1 "A - Government Hospital"
capture label values ca6a  provA_lbl
capture label values ca21a provA_lbl
capture label variable ca6a  "Diarrhea care source: Government Hospital (A)"
capture label variable ca21a "Fever/ARI care source: Government Hospital (A)"

capture label drop provB_lbl
label define provB_lbl 0 "Not mentioned" 1 "B - Government Health Centre"
capture label values ca6b  provB_lbl
capture label values ca21b provB_lbl
capture label variable ca6b  "Diarrhea care source: Government Health Centre (B)"
capture label variable ca21b "Fever/ARI care source: Government Health Centre (B)"

capture label drop provC_lbl
label define provC_lbl 0 "Not mentioned" 1 "C - Government Health Post"
capture label values ca6c  provC_lbl
capture label values ca21c provC_lbl
capture label variable ca6c  "Diarrhea care source: Government Health Post (C)"
capture label variable ca21c "Fever/ARI care source: Government Health Post (C)"

capture label drop provD_lbl
label define provD_lbl 0 "Not mentioned" 1 "D - Community Health Worker (Govt)"
capture label values ca6d  provD_lbl
capture label values ca21d provD_lbl
capture label variable ca6d  "Diarrhea care source: Community Health Worker-Govt (D)"
capture label variable ca21d "Fever/ARI care source: Community Health Worker-Govt (D)"

capture label drop provE_lbl
label define provE_lbl 0 "Not mentioned" 1 "E - Mobile / Outreach Clinic"
capture label values ca6e  provE_lbl
capture label values ca21e provE_lbl
capture label variable ca6e  "Diarrhea care source: Mobile/Outreach Clinic (E)"
capture label variable ca21e "Fever/ARI care source: Mobile/Outreach Clinic (E)"

capture label drop provH_lbl
label define provH_lbl 0 "Not mentioned" 1 "H - Other Public Medical"
capture label values ca6h  provH_lbl
capture label values ca21h provH_lbl
capture label variable ca6h  "Diarrhea care source: Other Public Medical (H)"
capture label variable ca21h "Fever/ARI care source: Other Public Medical (H)"

* PRIVATE MEDICAL SECTOR
capture label drop provI_lbl
label define provI_lbl 0 "Not mentioned" 1 "I - Private Hospital / Clinic"
capture label values ca6i  provI_lbl
capture label values ca21i provI_lbl
capture label variable ca6i  "Diarrhea care source: Private Hospital/Clinic (I)"
capture label variable ca21i "Fever/ARI care source: Private Hospital/Clinic (I)"

capture label drop provJ_lbl
label define provJ_lbl 0 "Not mentioned" 1 "J - Private Physician"
capture label values ca6j  provJ_lbl
capture label values ca21j provJ_lbl
capture label variable ca6j  "Diarrhea care source: Private Physician (J)"
capture label variable ca21j "Fever/ARI care source: Private Physician (J)"

capture label drop provK_lbl
label define provK_lbl 0 "Not mentioned" 1 "K - Private Pharmacy"
capture label values ca6k  provK_lbl
capture label values ca21k provK_lbl
capture label variable ca6k  "Diarrhea care source: Private Pharmacy (K)"
capture label variable ca21k "Fever/ARI care source: Private Pharmacy (K)"

capture label drop provL_lbl
label define provL_lbl 0 "Not mentioned" 1 "L - Community Health Worker (NGO)"
capture label values ca6l  provL_lbl
capture label values ca21l provL_lbl
capture label variable ca6l  "Diarrhea care source: CHW-NGO (L)"
capture label variable ca21l "Fever/ARI care source: CHW-NGO (L)"

capture label drop provM_lbl
label define provM_lbl 0 "Not mentioned" 1 "M - Mobile Clinic (NGO)"
capture label values ca6m  provM_lbl
capture label values ca21m provM_lbl
capture label variable ca6m  "Diarrhea care source: Mobile Clinic-NGO (M)"
capture label variable ca21m "Fever/ARI care source: Mobile Clinic-NGO (M)"

capture label drop provN_lbl
label define provN_lbl 0 "Not mentioned" 1 "N - NGO Clinic / Hospital"
capture label values ca6n  provN_lbl
capture label values ca21n provN_lbl
capture label variable ca6n  "Diarrhea care source: NGO Clinic/Hospital (N)"
capture label variable ca21n "Fever/ARI care source: NGO Clinic/Hospital (N)"

capture label drop provO_lbl
label define provO_lbl 0 "Not mentioned" 1 "O - Other Private Medical"
capture label values ca6o  provO_lbl
capture label values ca21o provO_lbl
capture label variable ca6o  "Diarrhea care source: Other Private Medical (O)"
capture label variable ca21o "Fever/ARI care source: Other Private Medical (O)"

capture label drop provW_lbl
label define provW_lbl 0 "Not mentioned" 1 "W - DK Public or Private"
capture label values ca6w  provW_lbl
capture label values ca21w provW_lbl
capture label variable ca6w  "Diarrhea care source: DK Public or Private (W)"
capture label variable ca21w "Fever/ARI care source: DK Public or Private (W)"

* OTHER SOURCES
capture label drop provP_lbl
label define provP_lbl 0 "Not mentioned" 1 "P - Relative / Friend"
capture label values ca6p  provP_lbl
capture label values ca21p provP_lbl
capture label variable ca6p  "Diarrhea care source: Relative/Friend (P)"
capture label variable ca21p "Fever/ARI care source: Relative/Friend (P)"

capture label drop provQ_lbl
label define provQ_lbl 0 "Not mentioned" 1 "Q - Shop / Market / Street"
capture label values ca6q  provQ_lbl
capture label values ca21q provQ_lbl
capture label variable ca6q  "Diarrhea care source: Shop/Market/Street (Q)"
capture label variable ca21q "Fever/ARI care source: Shop/Market/Street (Q)"

capture label drop provR_lbl
label define provR_lbl 0 "Not mentioned" 1 "R - Traditional Practitioner"
capture label values ca6r  provR_lbl
capture label values ca21r provR_lbl
capture label variable ca6r  "Diarrhea care source: Traditional Practitioner (R)"
capture label variable ca21r "Fever/ARI care source: Traditional Practitioner (R)"

capture label drop provS_lbl
label define provS_lbl 0 "Not mentioned" 1 "S - Charms / Spiritual / Other"
capture label values ca6s  provS_lbl
capture label values ca21s provS_lbl
capture label variable ca6s  "Diarrhea care source: Charms/Spiritual (S)"
capture label variable ca21s "Fever/ARI care source: Charms/Spiritual (S)"

di "All provider variable labels applied."

* Generic Yes/No label
...
di "Outcome distribution (weighted) — provider_choice:"
svy: tab provider_choice

di "Outcome distribution (weighted) — care_sought:"
svy: tab care_sought
...
notes: Outcomes: provider_choice (multinomial, 0-3) and care_sought (binary, 0-1)
